...
export function Sidebar() {
  ...
  // ✅ DYNAMIC ROUTES
  // Dashboard & Network link to specific project if inside one, else global
  const dashboardHref = projectId ? `/projects/${projectId}/dashboard` : "/dashboard";
  
  // ✅ REPORT LINKS
  const executiveCaseHref = "/reports/treasury-view"; // Corrected to match file structure
  const engineeringProgHref = "/reports/budget";      // Corrected
  const reportsBuilderHref = "/reports";              // Corrected
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "flex items-center h-16 px-4 border-b border-slate-100 dark:border-slate-800/50",
      ...
      <nav className="flex-1 space-y-1 p-3 overflow-y-auto no-scrollbar">
        ...
        >
          ...
          {/* AI Advisor: Only show if inside a project context */}
          {projectId && (
             <NavItem 
          ...
          {/* Governance: Global view or Project Specific */}
          <NavItem 
        ...
        {/* ✅ PROVINCIAL REPORTING (Outcome Driven) */}
        <NavGroup
          label="Provincial Reporting"
          icon={<Briefcase className="w-5 h-5" />}
          active={pathname.startsWith("/reports") || pathname.includes("/network")}
        ...
        >
          <NavItem
            href={executiveCaseHref}
            label="Executive Case"
            icon={<Presentation className="w-3 h-3" />}
          />

          <NavItem
            href={engineeringProgHref}
            label="Engineering Prog."
