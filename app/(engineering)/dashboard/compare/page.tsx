...
import { LineChart, FolderSearch, ArrowRight, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";

export default function GlobalCompareSelectPage() {
  ...
  const handleSelect = (id: string) => {
    router.push(`/projects/${id}/dashboard/compare`);
  };
    ...
    <div className="h-full w-full flex flex-col items-center justify-center p-8 bg-[var(--background)] animate-in fade-in duration-500">
      <div className="max-w-2xl w-full text-center space-y-6">
        ...
        <div className="w-20 h-20 mx-auto bg-[color:color-mix(in_oklab,var(--accent-color)_10%,transparent)] rounded-full flex items-center justify-center mb-6">
          <LineChart className="w-10 h-10 text-[var(--accent-color)]" />
        </div>
        
        <h1 className="text-3xl font-bold text-[var(--foreground)]">Comparative Analysis</h1>
        <p className="text-[color:color-mix(in_oklab,var(--foreground)_60%,transparent)] max-w-lg mx-auto leading-relaxed">
          To run a multi-scenario comparison and calculate unfunded liabilities, please select a workspace from your active projects below.
        </p>
