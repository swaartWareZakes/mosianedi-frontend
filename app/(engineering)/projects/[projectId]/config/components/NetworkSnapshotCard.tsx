...
import { Activity, Coins, Layers, BarChart3, RefreshCw, Info, Route as RouteIcon } from "lucide-react";
...
function fmtNum(val: any) {
  ...
  return v.toLocaleString("en-ZA", { maximumFractionDigits: 1 });
}

// Theme-stable helpers
...
export function NetworkSnapshotCard({ projectId }: { projectId: string }) {
  ...
  return (
    <div className={["bg-[var(--surface-bg)] rounded-2xl shadow-lg border overflow-visible relative", BORDER].join(" ")}>
      
      {/* Header */}
      <div className={["px-6 py-4 border-b flex justify-between items-center", BORDER_SOFT, BG_TINT].join(" ")}>
        <div>
          <h2 className="text-base font-bold text-[var(--foreground)] flex items-center gap-2">
            {isRoute ? <RouteIcon className="w-4 h-4 text-indigo-500" /> : <Activity className="w-4 h-4 text-indigo-500" />}
            {isRoute ? "Linear Asset Profile" : "Asset Health Profile"}
          </h2>
        ...
        <button onClick={refetch} disabled={loading} className={["transition-colors", TEXT_SOFT, "hover:text-indigo-500"].join(" ")} aria-label="Refresh snapshot">
      ...
      {loading ? (
        ...
        <div className={["divide-y", DIVIDE].join(" ")}>
          ...
          <div className="p-6">
            <div className="flex items-center gap-2 mb-1 relative">
              ...
              <div className="group relative ml-auto cursor-help">
                <Info className={["w-4 h-4 transition-colors", "text-[color:color-mix(in_oklab,var(--foreground)_30%,transparent)]", "group-hover:text-indigo-500"].join(" ")} />
                <div className={["absolute right-0 bottom-full mb-2 w-64 p-3 rounded-lg shadow-xl z-50", "opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none", "bg-[var(--surface-bg)] border", BORDER, "text-[10px]", "text-[var(--foreground)]"].join(" ")}>
                  <strong className="text-indigo-500">Methodology:</strong>
                  <br />
                  (Paved km × R3.5m) + (Gravel km × R0.25m)
                  <span className={["mt-1 block italic", TEXT_SOFT].join(" ")}>Based on COTO standard replacement rates.</span>
                  <div className={["absolute top-full right-1 -mt-1 w-2 h-2 rotate-45", "bg-[var(--surface-bg)] border border-t-0 border-l-0", BORDER].join(" ")} />
                </div>
              </div>
            </div>
            ...
            <p className={["text-xs mt-1", TEXT_SOFT].join(" ")}>
              Estimated replacement cost based on {fmtNum(totalLength)} km {isRoute ? 'route' : 'network'}.
            </p>
          </div>
          ...
          <div className="p-6">
            <div className="flex justify-between items-end mb-2 relative">
              <div className="flex items-center gap-2">
                ...
                <span className={["text-xs font-bold uppercase tracking-wider", TEXT_MUTED].join(" ")}>
                  {isRoute ? "Route Condition (VCI)" : "Network Condition (VCI)"}
                </span>

                {/* Tooltip */}
                <div className="group relative cursor-help">
                  <Info className={["w-4 h-4 transition-colors", "text-[color:color-mix(in_oklab,var(--foreground)_30%,transparent)]", "group-hover:text-indigo-500"].join(" ")} />
                  <div className={["absolute left-0 bottom-full mb-2 w-64 p-3 rounded-lg shadow-xl z-50", "opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none", "bg-[var(--surface-bg)] border", BORDER, "text-[10px]", "text-[var(--foreground)]"].join(" ")}>
                    <strong className="text-indigo-500">Visual Condition Index:</strong>
                    <br />0-30: Critical/Poor<br />30-50: Warning/Fair<br />50-85: Good<br />85+: Excellent<br />
                    <span className={["mt-1 block italic", TEXT_SOFT].join(" ")}>
                      {isRoute ? "Exact condition for this specific linear asset." : "Calculated as weighted average of all segments."}
                    </span>
                    <div className={["absolute top-full left-1 -mt-1 w-2 h-2 rotate-45", "bg-[var(--surface-bg)] border border-t-0 border-l-0", BORDER].join(" ")} />
                  </div>
                </div>
              </div>

              <div className="text-right">
                <span className="text-2xl font-bold text-[var(--foreground)]">{vci.toFixed(1)}</span>
            ...
            <div className={["h-3 w-full rounded-full overflow-hidden relative", BG_TINT2].join(" ")}>
              <div className={`h-full ${healthColor} transition-all duration-1000 ease-out`} style={{ width: `${Math.min(vci, 100)}%` }} />
            </div>
          ...
          {/* 3. ASSET MIX (Dynamic based on scope) */}
          <div className={["p-6", BG_TINT].join(" ")}>
            <div className="flex items-center gap-2 mb-4">
              <Layers className={["w-4 h-4", TEXT_SOFT].join(" ")} />
              <span className={["text-xs font-bold uppercase tracking-wider", TEXT_MUTED].join(" ")}>
                {isRoute ? "Surface Detail" : "Asset Composition"}
              </span>
            </div>

            <div className={`grid ${isRoute ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
              
              {/* PAVED */}
              {(!isRoute || pavedLength > 0) && (
                <div className={["p-3 rounded-xl border shadow-sm bg-[var(--surface-bg)]", BORDER_SOFT].join(" ")}>
                    <div className={["text-xs mb-1", TEXT_SOFT].join(" ")}>Paved {isRoute ? 'Length' : 'Network'}</div>
                    <div className="text-lg font-bold text-[var(--foreground)]">
                    {fmtNum(pavedLength)} <span className={["text-xs font-normal", TEXT_SOFT].join(" ")}>km</span>
                    </div>
              ...
              {/* GRAVEL */}
              {(!isRoute || gravelLength > 0) && (
                <div className={["p-3 rounded-xl border shadow-sm bg-[var(--surface-bg)]", BORDER_SOFT].join(" ")}>
                    <div className={["text-xs mb-1", TEXT_SOFT].join(" ")}>Gravel {isRoute ? 'Length' : 'Network'}</div>
                    <div className="text-lg font-bold text-[var(--foreground)]">
                    {fmtNum(gravelLength)} <span className={["text-xs font-normal", TEXT_SOFT].join(" ")}>km</span>
                    </div>
                    <div className={["h-1 w-full mt-2 rounded-full overflow-hidden", BG_TINT2].join(" ")}>
