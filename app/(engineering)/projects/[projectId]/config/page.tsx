...
import { ArrowLeft, ArrowRight, LayoutDashboard, Loader2, Route as RouteIcon, MapPin, Building2, Map } from "lucide-react";
...
export default function ProjectWizardPage() {
  ...
  // Dynamic Location Tag for Header
  ...
  return (
    <div className="h-full w-full bg-[var(--background)] flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={["hidden lg:block w-72 overflow-y-auto border-r", "bg-[var(--surface-bg)]", BORDER_SOFT].join(" ")}>
          <div className="p-6">
            <Link href="/projects" className={["inline-flex items-center gap-2 text-xs font-bold mb-8 transition-colors", TEXT_MUTED, "hover:text-[var(--accent-color)]"].join(" ")}>
            ...
            <div className="mb-2">
              ...
              <span className={["inline-flex items-center gap-1.5 text-xs font-mono px-2 py-1 rounded border", BG_SOFT, BORDER_SOFT_2, TEXT_MUTED].join(" ")}>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2">
                  ...
                  <p className={TEXT_MUTED}>
                    Define the physical inventory and traffic conditions. This sets the baseline for the simulation.
                  </p>
                </div>
                ...
                {proposalLoading ? (
                  ...
                  <ProposalInputsCard
                    proposal={proposal}
                    projectMeta={projectMeta} // Passing the Meta down!
                    saving={false}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2">
                  ...
                  <p className={TEXT_MUTED}>Review the calculated Asset Value (CRC) and Visual Condition Index (VCI) before forecasting.</p>
                </div>
                <NetworkSnapshotCard projectId={projectId} />
                <div className={["pt-4 border-t", BORDER_SOFT].join(" ")}>
                  <h3 className={["text-sm font-bold uppercase tracking-wider mb-6", TEXT_MUTED].join(" ")}>Visual Breakdown</h3>
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2">
                  ...
                  <p className={TEXT_MUTED}>Adjust economic variables and deterioration rates to shape the long-term outlook.</p>
                </div>
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2">
                  ...
                  <p className={TEXT_MUTED}>Execute the RoNET engine. This will generate the final budget requirements and condition forecast.</p>
                </div>
      ...
      <div className={["sticky bottom-0 p-6 z-40 border-t", "bg-[var(--surface-bg)]", BORDER_SOFT].join(" ")}>
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <button onClick={handleBack} disabled={step === 1 || navigating} className={["px-6 py-3 rounded-xl font-bold transition-colors", "text-[color:color-mix(in_oklab,var(--foreground)_70%,transparent)]", BG_HOVER, "disabled:opacity-30 disabled:hover:bg-transparent"].join(" ")}>
            Back
          </button>

          {step < 4 ? (
            <button onClick={handleNext} disabled={navigating} className="group flex items-center gap-2 px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-500/20 transition-all hover:scale-105 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
            ...
            <button onClick={handleFinish} className="flex items-center gap-2 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95">
