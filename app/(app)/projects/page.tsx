...
export default function ProjectsPage() {
  ...
  async function loadData() {
    ...
    // We map manually to handle the potential "No Assignee" case gracefully
    ...
    const mergedProjects = (projectsData || []).map((p: any) => ({
        ...p,
        status: p.status || 'backlog', // Default to backlog
  ...
  return (
    <div className="h-full w-full bg-[var(--background)] p-6 overflow-y-auto"> {/* Changed container styles */}
      <div className="max-w-7xl mx-auto space-y-8 pb-10"> {/* Added padding bottom */}
        
        {/* HEADER */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Mission Control</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Manage strategic road programs and approval workflows.</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-1 gap-4">
            <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto no-scrollbar pb-2 sm:pb-0"> {/* Adjusted gap and padding */}
                <TabButton active={activeTab === "all"} onClick={() => setActiveTab("all")} label="All Projects" count={projects.length} />
                <TabButton active={activeTab === "mine"} onClick={() => setActiveTab("mine")} label="My Assignments" icon={<User className="w-4 h-4" />} />
                <TabButton active={activeTab === "planning"} onClick={() => setActiveTab("planning")} label="Active Planning" icon={<FileText className="w-4 h-4 text-blue-500" />} count={projects.filter(p => p.status === 'planning').length} />
                <TabButton active={activeTab === "review"} onClick={() => setActiveTab("review")} label="Needs Review" icon={<AlertCircle className="w-4 h-4 text-amber-500" />} count={projects.filter(p => p.status === 'review').length} />
                <TabButton active={activeTab === "backlog"} onClick={() => setActiveTab("backlog")} label="Backlog" icon={<Clock className="w-4 h-4 text-slate-400" />} count={projects.filter(p => p.status === 'backlog' || !p.status).length} />
            </div>
        ...
        {loading ? (
            ...
            <div className="py-20 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
                ...
                <p className="text-slate-500 mt-2">Try adjusting your filters or create a new program.</p>
            </div>
        ) : (
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)}>
          <ProjectCreateForm onClose={(created) => {
              setIsFormOpen(false);
              if (created) loadData(); // Reload list if project created
          }} />
      </Modal>
...
function TabButton({ active, onClick, label, count, icon }: any) {
    return (
        <button 
            onClick={onClick}
            className={cn(
                "flex items-center gap-2 px-3 py-2 text-sm font-medium transition-colors whitespace-nowrap rounded-lg", // Changed to pill style for better spacing handling
                active 
        ...
        >
            ...
            {count !== undefined && (
                <span className={cn(
                    ...
                    active ? "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"
                )}>
                    {count}
                </span>
            )}
