...
const formatMoney = (amount: number) => {
  ...
  if (v >= 1_000_000_000) return `R ${(v / 1_000_000_000).toFixed(2)}bn`;
  ...
  return (
    ...
    >
      ...
      <header className="border-b-4 border-indigo-600 pb-6 flex justify-between items-end">
        <div>
          <div className="text-indigo-600 font-extrabold text-xs uppercase tracking-[0.2em] mb-2">
            Decision Brief • Infrastructure Case
          </div>
          <h1 className="text-4xl font-black text-slate-900 leading-tight mb-2">
            {config.title || "Business Case"}
          </h1>
      ...
      {/* IMPACT GRAPH (simple bars for print robustness) */}
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-4 text-[10px] font-bold uppercase tracking-wider text-slate-600">
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-indigo-600" /> Funded Intervention
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-rose-400" /> Do Nothing (Decay)
            </div>
          </div>
        </div>
        ...
        <div className="h-48 flex items-end gap-3 px-2">
          {chartData.map((d: any, i: number) => (
            <div key={i} className="flex-1 flex flex-col justify-end relative h-full group">
              ...
              <div
                className="w-full bg-indigo-600 rounded-t-sm absolute bottom-0 left-0 right-0 shadow-lg"
                style={{ height: `${d.fundedVCI}%` }}
              />
        ...
        <div className="flex gap-6">
          <div className="w-1/3">
            <div className="bg-indigo-600 text-white p-6 rounded-xl h-full flex flex-col justify-between">
              ...
              <div>
                <div className="text-indigo-200 text-xs font-bold uppercase tracking-wider mb-2">
                  Strategic Driver
                </div>
                <div className="text-xl font-bold leading-snug">
                  “{String(narrative.executiveSummary || "").split(".")[0]}.”
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-2/3 space-y-4 text-sm leading-relaxed text-slate-600">
            ...
            <div className="p-4 bg-rose-50 border border-rose-100 rounded-lg text-rose-800 text-xs font-medium flex gap-3">
              ...
              <div>
                <strong className="block text-rose-900 mb-1">Risk Warning</strong>
        ...
        <div className="mt-auto">
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {chartData.slice(0, 5).map((row: any, i: number) => (
                <tr key={i}>
